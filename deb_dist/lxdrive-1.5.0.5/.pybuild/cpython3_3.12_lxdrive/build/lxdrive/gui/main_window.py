...
class AccountWidget(QFrame):
    ...
    def _setup_ui(self):
        ...
        # Icono del servicio (usar logo si está disponible)
        icon_label = QLabel()
        icon_label.setFixedSize(50, 50)
        logo_path = Path(__file__).parent.parent / 'img' / 'logo.png'
        pix = QPixmap(str(logo_path))
        if not pix.isNull():
            icon_label.setPixmap(pix.scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        else:
            # Fallback a texto si no hay imagen
            icon_label.setText(self._get_service_icon())
        icon_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        icon_label.setStyleSheet("""
            QLabel { border-radius: 12px; }
        """)
        ...
        self.name_label.setFont(QFont("Segoe UI", 13, QFont.Weight.DemiBold))
        ...
        # Botones de acción (usar iconos del tema y fallback a logo)
        btn_layout = QHBoxLayout()
        btn_layout.setSpacing(8)
        
        # Botón sincronizar
        self.sync_btn = QPushButton()
        self.sync_btn.setIcon(self._get_icon("view-refresh"))
        ...
        mount_icon_name = "media-eject" if self.account.mount_enabled else "drive-harddisk"
        self.mount_btn.setIcon(self._get_icon(mount_icon_name))
        ...
        self.settings_btn.setIcon(self._get_icon("preferences-system"))
    ...
    def _get_service_icon(self) -> str:
        """Obtiene el icono según el tipo de servicio (fallback a texto)"""
        icons = {
            "drive": "G",
            "dropbox": "D",
            "onedrive": "O",
    ...
    def _get_icon(self, name: str) -> QIcon:
        """Obtiene un QIcon a partir del tema o, si no existe, del logo incluido"""
    ...
    def update_account(self, account: Account):
        ...
        if account.mount_enabled:
            self.mount_btn.setIcon(self._get_icon("media-eject"))
        else:
            self.mount_btn.setIcon(self._get_icon("drive-harddisk"))
...
class AddAccountDialog(QDialog):
    ...
    def _add_account(self):
            ...
            for r in remotes:
                if r.name == remote_name:
                    ...
                    try:
                        ...
                        logger.error(f"Tipo de remote desconocido: {remote_type_str}")
                        self._show_styled_message("error", "Error", f"El tipo de remote '{remote_type_str}' no es soportado.")
                        return
                    break
        else:
            remote_type = cast(RemoteType, selected_data)
        ...
        # Asegurar que remote_type está definido
        if remote_type is None:
            self._show_styled_message("error", "Error", "No se pudo determinar el tipo de remote")
            return

        logger.debug(f"Remote type determinado: {remote_type}, tipo: {type(remote_type)}")
...
class AccountSettingsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        logo_path = Path(__file__).parent.parent / 'img' / 'logo.png'
        if QPixmap(str(logo_path)).isNull():
            title_icon.setText("⚙️")
        else:
            title_icon.setPixmap(QPixmap(str(logo_path)).scaled(20, 20, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        title_label = QLabel(self.account.name)
        ...
        sync_group.setStyleSheet("QGroupBox#syncGroup { background-color: #2f2f2f; border: 1px solid #4a4a4a; border-radius:8px; padding:8px; } QGroupBox#syncGroup::title { color: #ddd; subcontrol-origin: margin; left: 10px; padding: 0 5px; }")
        sync_layout = QFormLayout(sync_group)
        # Mejoras visuales: márgenes y espaciado más generosos
        sync_layout.setContentsMargins(12, 12, 12, 12)
        sync_layout.setHorizontalSpacing(20)
        sync_layout.setVerticalSpacing(8)
        sync_layout.setLabelAlignment(Qt.AlignmentFlag.AlignLeft)
        
        # Habilitar sincronización (más visible)
        self.sync_enabled = QCheckBox("Sincronización automática activa")
        self.sync_enabled.setChecked(self.account.sync_enabled)
        self.sync_enabled.setStyleSheet("padding: 4px 0; font-weight: 600;")
        sync_layout.addRow(self.sync_enabled)
        
        # Dirección (mejor ancho para evitar que el texto/emoji se corte)
        self.direction_combo = QComboBox()
        self.direction_combo.setMinimumWidth(220)
        self.direction_combo.setFixedHeight(28)
        self.direction_combo.addItem("Bidireccional", SyncDirection.BIDIRECTIONAL)
        self.direction_combo.addItem("Solo subir", SyncDirection.UPLOAD_ONLY)
        self.direction_combo.addItem("Solo descargar", SyncDirection.DOWNLOAD_ONLY)
        self.direction_combo.setToolTip("Selecciona la dirección de sincronización")

        # Log para debug: verificar items del combo
        logger.info(f"Direction combo items (sin emojis): {[self.direction_combo.itemText(i) for i in range(self.direction_combo.count())]}")
        ...
        # Intervalo (separar la unidad para evitar solapamientos del sufijo)
        self.interval_spin = QSpinBox()
        self.interval_spin.setRange(60, 3600)
        # No usar sufijo para mantener el control limpio; colocamos la unidad en una etiqueta
        self.interval_spin.setSuffix("")
        self.interval_spin.setValue(self.account.sync_interval)
        self.interval_spin.setFixedHeight(28)

        interval_cont = QWidget()
        interval_layout = QHBoxLayout(interval_cont)
        interval_layout.setContentsMargins(0, 0, 0, 0)
        interval_layout.setSpacing(8)
        interval_layout.addWidget(self.interval_spin)
        interval_layout.addWidget(QLabel("segundos"))
        interval_layout.addStretch()

        sync_layout.addRow("Intervalo:", interval_cont)
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        activity_manager=None,  # ActivityLogManager opcional
        config=None  # Config opcional para cuenta predeterminada
    ):
        super().__init__()
        ...
        self.bridge.activity_signal.connect(self._on_file_activity_ui)

        self._account_widgets: dict[str, AccountWidget] = {}  # account_id -> AccountWidget
        ...
        self._load_accounts()
        self._select_default_account()  # Seleccionar cuenta predeterminada
    ...
    def _select_default_account(self):
        """Selecciona la cuenta predeterminada al iniciar"""
        ...
        # Buscar cuenta predeterminada en config
        if self._config:
            default_account_id = self._config.get("default_account_id", "")
        
        # Si no hay predeterminada o no existe, usar la primera
        if not default_account_id or not self.account_manager.get_by_id(default_account_id):
            default_account_id = accounts[0].id
        
        # Seleccionar la cuenta
        self._select_account_by_id(default_account_id)
        
        # Sincronizar con el panel de actividad
        if self.activity_panel and hasattr(self.activity_panel, 'set_current_account'):
            self.activity_panel.set_current_account(default_account_id)
    ...
    def _setup_ui(self):
        ...
        """)
        self.mount_card.mousePressEvent = lambda a0: self._show_mount_details()
        ...
        # Conectar el ActivityLogManager al panel
        if self._activity_manager:
            self.activity_panel.set_activity_manager(self._activity_manager)
            # Configurar cuentas disponibles
            accounts = self.account_manager.get_all()
            if accounts:
                self.activity_panel.set_accounts(accounts)
        
        # Conectar cambio de cuenta en el panel con la UI principal
        self.activity_panel.account_changed.connect(self._on_activity_panel_account_changed)
    ...
    def _on_sync_start(self, account_id: str):
        ...
        self.bridge.start_signal.emit(account_id)

    def _on_sync_complete(self, task):
        """Callback para sync completada - emite señal al bridge"""
        self.bridge.complete_signal.emit(task)

    def _on_sync_error(self, account_id: str, message: str):
        """Callback para error de sync - emite señal al bridge"""
        self.bridge.error_signal.emit(account_id, message)

    def _handle_file_activity(self, account_id: str, name: str, action_str: str, path: str):
        """Callback para actividad de archivos - emite señal al bridge"""
        self.bridge.activity_signal.emit(account_id, name, action_str, path)
    ...
    def _load_accounts(self):
        ...
        for i in reversed(range(self.account_list_layout.count())):
            item = self.account_list_layout.itemAt(i)
            if item is not None:
                widget = item.widget()
    ...
    def _select_account(self, account, button):
        ...
        for i in range(self.account_list_layout.count()):
            ...
            if item is not None:
                widget = item.widget()
                if widget is not None and widget != button and isinstance(widget, QPushButton):
        ...
        # Sincronizar con el panel de actividad (sin recursión)
        if self.activity_panel and hasattr(self.activity_panel, 'set_current_account'):
            self.activity_panel.set_current_account(account.id, emit_signal=False)
        
        # Guardar como cuenta predeterminada
        if self._config:
            self._config.set("default_account_id", account.id)
    
    def _on_activity_panel_account_changed(self, account_id: str):
        """Maneja el cambio de cuenta desde el panel de actividad"""
        # Evitar recursión: solo actualizar sidebar si la cuenta cambió realmente
        if self.selected_account and self.selected_account.id == account_id:
            return
        # Sincronizar la selección con el sidebar
    ...
    def _select_account_by_id(self, account_id: str):
        ...
        for i in range(self.account_list_layout.count()):
            ...
            if item is not None:
                widget = item.widget()
                if widget is not None and isinstance(widget, QPushButton) and widget.property("account_id") == account_id:
    ...
    def _load_sync_pairs(self, account):
        ...
        for i in reversed(range(self.sync_pairs_layout.count())):
            item = self.sync_pairs_layout.itemAt(i)
            if item is not None:
                widget = item.widget()
                if widget is not None:
                    widget.setParent(None)
        ...
        for pair in account.sync_pairs:
            ...
            """)
            pair_widget.mousePressEvent = lambda a0, p=pair: self._show_sync_pair_details(p)
    ...
    def _add_sync_pair_clicked(self):
        ...
        if dialog.exec():
            ...
            if local and remote:
                ...
                self.status_bar.showMessage(f"Carpeta añadida y sincronización iniciada", 3000)
    ...
    def _remove_sync_pair(self, account, pair):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self._load_sync_pairs(account)
            self.status_bar.showMessage("Carpeta desvinculada", 3000)
    ...
    def _mount_account_clicked(self):
        ...
        if is_active:
            ...
            if success:
                ...
                self.status_bar.showMessage(f"Unidad {account.name} desmontada", 3000)
            else:
                QMessageBox.warning(self, "Aviso al desmontar", msg)
        else:
            # Montar
            self.status_bar.showMessage(f"Montando {account.name}...", 0)
            ...
            if success:
                ...
                self.status_bar.showMessage(f"Unidad {account.name} montada correctamente", 3000)
            else:
                QMessageBox.critical(self, "Error al montar", msg)
                self.status_bar.showMessage("Error al montar unidad", 3000)
    ...
    def _show_mount_details(self):
        ...
        if self.selected_account is None: return
    ...
    def _on_file_activity(self, account_id: str, name: str, action_str: str, path: str):
        ...
        logger.debug(f"Actividad recibida -> Cuenta: {account_id}, Nombre: {name}, Acción: {action_str}, Ruta: {path}")
        if self.activity_panel is not None:
            from .activity_panel import FileActivity, FileAction

            # Filtrar actividades que no deben mostrarse como archivos individuales
            if action_str in ["sync_start", "sync_progress", "sync_complete"]:
                # No mostrar mensajes de estado como actividades de archivo
                logger.debug(f"Filtrando mensaje de estado: {action_str}")
                return
            ...
            if action_str in ["vfs_open", "vfs_read", "vfs_write"]:
                ...
                logger.debug(f"Actividad clasificada como VFS: {action_str}")
            elif action_str == "mounted" and name == "Unidad Virtual":
                is_mount_activity = True
                logger.debug(f"Actividad clasificada como VFS: montaje")
            ...
            account = self.account_manager.get_by_id(account_id)
            acc_name = account.name if account is not None else "Sistema"

            # 3. Añadir a la pestaña correcta
            logger.debug(f"Añadiendo actividad a {'VFS' if is_mount_activity else 'Carpetas'}: {name} - {action_str}")
            self.activity_panel.add_activity(FileActivity(
                path=path,
                name=name,
                action=action,
